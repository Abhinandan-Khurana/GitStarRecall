...
import { ArrowDown, ArrowRight, ChevronDown, Github, Lock, Sparkles } from "lucide-react";
...
export function LandingHero({
  ...
  return (
    <section className="relative min-h-screen overflow-hidden">
          ...
          <div className="hidden xl:block">
            <div className="relative mx-auto min-h-[680px] max-w-[1500px]">
              ...
              <div className="absolute inset-x-0 bottom-0 z-20 grid items-end gap-4 px-6 pb-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,0.75fr)]">
                <div className={cardEnterClass} style={reducedMotion ? undefined : { animationDelay: "800ms" }}>
                  <HeroCard title="Read-only GitHub access" className={`${hoverLiftClass} min-h-[9rem]`}>
                    <div className="mt-4 flex max-w-xl items-start gap-4">
                      ...
                      <p className="text-sm leading-7 text-muted-foreground">
                        GitHub OAuth requests public-repo read-only access, and PAT fallback uses equivalent read-only repository access. GitStarRecall never asks for write access.
                      </p>
                    </div>
                  </HeroCard>
                </div>

                <div className={`flex flex-col items-end gap-3 ${buttonsEnterClass}`} style={reducedMotion ? undefined : { animationDelay: "900ms" }}>
                  <Button size="lg" className="rounded-full px-6 shadow-[0_12px_32px_hsl(var(--primary)/0.2)]" onClick={onScrollToConnect}>
                    Get started
                    <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                  <Button size="lg" variant="outline" className="rounded-full px-6" onClick={onScrollToFlow}>
            ...
            <div className={`mt-4 flex justify-center ${buttonsEnterClass}`} style={reducedMotion ? undefined : { animationDelay: "1100ms" }}>
              <button type="button" onClick={onScrollToFlow} className="flex flex-col items-center gap-1 text-muted-foreground/60 transition-colors hover:text-primary" aria-label="Scroll to explore">
          ...
          <div className="xl:hidden">
            ...
            <div className="relative z-10 mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-[minmax(0,0.76fr)_minmax(0,1.1fr)_minmax(0,0.9fr)]">
              ...
              <div className="md:col-span-2 xl:col-span-1" style={reducedMotion ? undefined : { transform: `translateY(${heroOffsetY * 0.28}px)` }}>
                <div className={`h-full ${floatCardClassC}`} style={reducedMotion ? undefined : { animationDelay: "1400ms" }}>
                  <div className={`h-full ${cardEnterClass}`} style={reducedMotion ? undefined : { animationDelay: "600ms" }}>
                    <HeroCard title="Read-only GitHub access" className={`${hoverLiftClass} h-full`}>
                      ...
                      <div className="mt-5 flex flex-wrap gap-2 text-xs text-muted-foreground">
                        <span className="rounded-full border border-border bg-muted px-3 py-1">OAuth</span>
                        <span className="rounded-full border border-border bg-muted px-3 py-1">PAT</span>
                        <span className="rounded-full border border-border bg-muted px-3 py-1">No write scope</span>
                      </div>
                    </HeroCard>
                  </div>
                </div>
      ...
      <div className="pointer-events-none fixed inset-x-0 top-0 z-50 mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
        <div className={`pointer-events-auto flex items-center justify-between py-4 transition-all duration-300 ${
          scrolled
